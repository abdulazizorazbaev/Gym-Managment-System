...
{
    /// <summary>
    /// Interaction logic for InstructorUpdateWindow.xaml
    /// </summary>
    public partial class InstructorUpdateWindow : Window
    {
        private readonly IInstructorRepository _instructorRepository;
        private readonly IPackageRepository _packageRepository;
        public long Id { get; set; }

        public InstructorUpdateWindow()
        {
            InitializeComponent();
        ...
        {
            tbFirstName.Text = instructor.FirstName;
            tbLastName.Text = instructor.LastName;
            string imagePath = instructor.ImagePath;
            ImbBImage.ImageSource = new BitmapImage(new Uri(imagePath, UriKind.Relative));
            tbEmail.Text = instructor.Email;
            dtpDateOfBirth.Text = instructor.Date_of_birth.ToString();
            tbTitle.Text = instructor.Title;
            cmbPackageList.Text = instructor.PackageId.ToString();
            tbSalary.Text = instructor.Salary.ToString();
            string str = instructor.PhoneNumber;
            tbPhoneNumber.Text = str.Substring(4);
            rbDescription.Document.Blocks.Clear();
            rbDescription.Document.Blocks.Add(new Paragraph(new Run(instructor.Description)));
            Id = instructor.Id;
        }
        ...
        public async void DeleteAsync(Instructor instructor)
        {
            MessageBoxResult result = MessageBox.Show("Are you sure to delete the instructor?","Warning message!",MessageBoxButton.YesNo);
            if (result == MessageBoxResult.Yes)
            {
                var res = await _instructorRepository.DeleteAsync(instructor.Id);
                if (res != 0)
                {
                    MessageBox.Show("The instructor has been successfully deleted!");
                }
            }
        }
        ...
        private async void btnUpdate_Click(object sender, RoutedEventArgs e)
        {
            ...
            var result = await _instructorRepository.UpdateAsync(Id, instructor);
            if (result > 0)
            {
        ...
        private void brInstructorUpdateDrager_MouseDown(object sender, MouseButtonEventArgs e)
        {
        ...
        private void btnInstructorUpdateExit_Click(object sender, RoutedEventArgs e)
        {
            Close();
        }
    }
